...
// systemdRecord - standard record (struct) for linux systemd
...
func (linux *systemDRecord) checkRunning() (string, bool) {
	output, err := exec.Command("systemctl", "status",
		linux.name+".service").Output()

	if err == nil {
		if matched, err := regexp.MatchString("Active: active",
			string(output)); err == nil && matched {
...
func (linux *systemDRecord) Install(args ...string) (string, error) {
	...
	if err := exec.Command("systemctl", "enable",
		linux.name+".service").Run(); err != nil {
...
func (linux *systemDRecord) Remove() (string, error) {
	...
	if err := exec.Command("systemctl", "disable",
		linux.name+".service").Run(); err != nil {
...
func (linux *systemDRecord) Start() (string, error) {
	...
	if err := exec.Command("systemctl", "start",
		linux.name+".service").Run(); err != nil {
...
func (linux *systemDRecord) Stop() (string, error) {
	...
	if err := exec.Command("systemctl", "stop",
		linux.name+".service").Run(); err != nil {
